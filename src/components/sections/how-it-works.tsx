        ...
        <section className="py-20 bg-white border-b border-slate-200" id="howItWorks">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                    ...
                    <p className="mt-2 text-sm text-slate-500">Click each step to learn more.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                    {steps.map((step, index) => (
                        <div key={index} className="h-full fade-in-up" style={{ animationDelay: `${index * 0.15}s` }}>
                            <Link href={step.href} onClick={(e) => handleClick(e, step.href)} className="block h-full">
                                <Card className="h-full bg-slate-50 border-slate-100 hover:border-primary/30 hover:bg-white hover:shadow-xl transition-all flex flex-col group hover:-translate-y-2 text-left">
